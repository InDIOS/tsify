...
module.exports = function (ts) {
	...
	function parseOptions(opts, bopts) {
		...
		// Get the preprocess option
		parsed.options.preprocess = opts.preprocess || false;
		// Get the vue option
		parsed.options.vueOptions = opts.vueOptions || null;
	...
	Tsifier.prototype.transform = function (file) {
		...
		function flush(next) {
			...
			switch (fileType) {
				case 'sty':
					compiled = self.getStyleFile(file);
					break;
				case 'tpl':
					compiled = self.getTemplateFile(file);
					break;
				default:
					compiled = self.getCompiledFile(file);
			}
			if (compiled) {
				this.push(compiled);
			}
	...
	Tsifier.prototype.getTemplateFile = function (inputFile) {
		var self = this;
		var output = self.host._template(inputFile);
		if (output === undefined) {
			self.emit('error', new Error('tsify: no template file for ' + inputFile));
			trace('Template file %s not found.', inputFile);
			return;
		}
	...
	Tsifier.prototype.getStyleFile = function (inputFile) {
		var self = this;
		var output = self.host._style(inputFile);
		if (output === undefined) {
			self.emit('error', new Error('tsify: no style file for ' + inputFile));
			trace('Style file %s not found.', inputFile);
			return;
		}
		output = 'module.exports=' + JSON.stringify(output) + ';';
		return output;
