...
module.exports = function(ts) {
  function Host(currentDirectory, opts) {
	...
	Host.prototype.getHmrPartial = function (type) {
		if (process.env.NODE_ENV !== 'production') {
      return fs.readFileSync(path.join(__dirname, 'hmr-partial-' + type + '.js'), 'utf-8');
		} else {
			return '';
		}
	};
  ...
  Host.prototype._addFile = function(filename, root) {
    ...
    this.files[canonical] = {
      ...
      root: root,
      version: version,
      nodeModule: /\/node_modules\/(?!typescript\/)/.test(canonical) && !/\.d\.ts$/i.test(canonical)
    };
  ...
  Host.prototype._output = function(filename) {
    ...
    if (output) {
      ...
      var isVueComp = output.match(/(?:vue_ts_decorate_[0-9]\.)(?:Component|Directive|Filter|Mixin)/g) !== null;
			if (!tsd.match(outputCanonical) && (this.isInclude(filename) || isVueComp)) {
				var hmrPartial;
				if (isVueComp) {
					hmrPartial = this.getHmrPartial('vue')
						.replace(/HOTRELOADAPI/g, location('vue-hot-reload-api'))
						.replace(/COMP_ID/g, hash(filename));
				} else {
					hmrPartial = this.getHmrPartial('ud')
  ...
  Host.prototype.isInclude = function(filename) {
    var isInclude = false;
    if (this.vue.includeHmrFiles) {
      var includeFiles = Array.isArray(this.vue.includeHmrFiles) ? this.vue.includeHmrFiles : [this.vue.includeHmrFiles];
      for (var i = 0, len = includeFiles.length; i < len; i++) {
        var pattern = includeFiles[i];
  ...
  function replacer(host, filename, type) {
    return function(content, relativePath) {
      ...
      if (!host.fileExists(dir) && /(\.css$|\.html$)/.test(ext)) {
        return type + ': \'\'';
      }
